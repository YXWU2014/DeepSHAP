...
def preprocessed_data():
    ...
    del X["SGOT_isBlankbutapplicable"]
    # would have no variance in the strain set
    del X["calcium_isBlankbutapplicable"]
    # would only have one true value in the train set
    del X["uric_acid_isBlankbutapplicable"]
    # would only have one true value in the train set
    del X["urine_hematest_isVerylarge"]
    # would only have one true value in the train set
    del X["total_bilirubin_isBlankbutapplicable"]
    # would only have one true value in the train set
    del X["alkaline_phosphatase_isBlankbutapplicable"]
    # redundant with hematocrit_isUnacceptable
    del X["hemoglobin_isUnacceptable"]
    nan_rows = np.isnan(X["height"]) | np.isnan(
        X["weight"]) | np.isnan(X["systolic_blood_pressure"])
...
def load_nhanes_new(is_cycle_split=True, val_size=0.2):
    ...
    if is_cycle_split:  # Split trval-test by release cycle
        ...
        X_trval, X_test, y_trval, y_test = train_test_split(
            X, y, test_size=0.2, random_state=12312)

    X_train, X_valid, y_train, y_valid = train_test_split(
        X_trval, y_trval, test_size=val_size, random_state=4212312)
...
def load_gene_expr(outcome_name):
    ...
    if outcome_name == "alzheimers":
        ...
        # Load data
        y_data = pd.read_csv(
            DPATH+"AD_DATA/ROSMAP_GE1_Diagnosis_Labels.tsv", sep='\t')
        X_data = pd.read_csv(
            DPATH+"AD_DATA/ROSMAP_GE1_Preprocessed_Expression_Matrix.tsv", sep='\t')
        ...
        # Split data
        X_trval, X_test,  y_trval, y_test = train_test_split(
            X_data, y_data, test_size=0.2, random_state=23)
        X_train, X_valid, y_train, y_valid = train_test_split(
            X_data, y_data, test_size=0.3, random_state=12)
        ...
        X_data = pd.read_csv(
            DPATH+"CANCER_DATA/METABRIC_Expression_NORMALIZED.tsv", sep='\t')
        ...
        if outcome_name == "breastcancer_er":
            ...
            # Use tumor grade (from microscope) as outcome
            y_data = y_data["Neoplasm Histologic Grade"]
